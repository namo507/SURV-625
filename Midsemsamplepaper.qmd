...
To ensure that each cluster has a minimum Measure of Size (MOS) of at least 35, we need to form linked clusters for any block with an MOS below this threshold. We will iterate through the list and combine consecutive blocks until the cumulative MOS for a cluster is 35 or greater. Here's how we can form the linked clusters and fill in the “Cluster Number” column: • Block 1: MOS = 50 (\>= 35). This forms Cluster 1.

• Block 2: MOS = 20 (\< 35). This needs to be linked with subsequent blocks. ◦ Block 2 (MOS 20) + Block 3 (MOS 25) = Cumulative MOS 45 (\>= 35). These two blocks form Cluster 2.
...
Determine the valid range for a random start: The valid range for a random start (RN) is from 1 to k, inclusive.

Therefore, the valid range is 1 to 183.33. Since the random start should be an integer, the valid range for the random start is 1 to 183.
...
We know each cluster's selection probability (π) We calculate each cluster's subsampling rate (r) using:

r = (overall rate) ÷ (cluster probability)
...
Step 2: Set Up the Systematic Selection Total MOS = 400

We need to select 2 clusters Selection interval = 400 ÷ 2 = 200

Random start = 75 (given in the problem)

Step 3: Make the Selections Think of the MOS as stretching along a number line from 1 to 400. Each cluster occupies a portion:
...
Cluster 2 (Blocks 2 & 3): Size (MOS) = 73

Calculation: (2 × 73) ÷ 400 = 146 ÷ 400 = 0.365 (about 36.5%)

This means Cluster 2 had about a 36.5% chance of being selected

Cluster 4 (Blocks 7, 8 & 9): Size (MOS) = 128

Calculation: (2 × 128) ÷ 400 = 256 ÷ 400 = 0.64 (64%)

This means Cluster 4 had a 64% chance of being selected

Why This Makes Sense Cluster 4 is almost twice as large as Cluster 2 (128 vs 73), so it had almost twice the chance of being selected (64% vs 36.5%). This shows how PPS sampling works - larger clusters have proportionally larger chances of being selected.
...
The Simple Formula For EPSEM sampling, we use this formula:

Within-cluster selection rate = Overall sampling rate ÷ Cluster selection probability
...
For Our Selected Clusters Cluster 2 (Blocks 2 & 3): Selection probability: 0.365 (36.5%)

Within-cluster rate = 0.05 ÷ 0.365 = 0.137 (about 13.7%)

This means we need to sample about 13.7% of housing units in Cluster 2

Cluster 4 (Blocks 7, 8 & 9): Selection probability: 0.64 (64%) \# Define the data cluster_totals \<- c(10,15,5,5,16,8,8,15,9,9,9,10,4,3,11,12,8,4,8,6) n \<- length(cluster_totals) \# Number of sampled hospitals N \<- 189500 \# Total number of hospitals M \<- 20 \# Number of births per hospital total_pop \<- 3790000 \# Total population size

# Basic statistics

mean_per_cluster \<- mean(cluster_totals) sd_clusters \<- sd(cluster_totals)# Calculate the estimate p_hat \<- sum(cluster_totals)/(n\*M)

# Display result

cat("Estimated proportion of children in fragile families:", p_hat, "\n")# Calculate the variance of cluster proportions cluster_props \<- cluster_totals/M var_props \<- var(cluster_props)

# Calculate standard error with finite population correction

fpc \<- sqrt((N-n)/N) se_p \<- sqrt(var_props/n) \* fpc

# Display result

cat("Standard error of the proportion estimate:", se_p, "\n")# Calculate the variance of cluster proportions cluster_props \<- cluster_totals/M var_props \<- var(cluster_props)
...
cat("Standard error of the proportion estimate:", se_p, "\n")# Calculate 95% confidence interval t_value \<- qt(0.975, df=n-1) ci_lower \<- p_hat - t_value \* se_p ci_upper \<- p_hat + t_value \* se_p

# Display result

cat("95% confidence interval:", c(ci_lower, ci_upper), "\n")# Calculate estimated total Y_hat \<- N \* M \* p_hat

# Display result

cat("Estimated total number of children in fragile families:", Y_hat, "\n") Within-cluster rate = 0.05 ÷ 0.64 = 0.078 (about 7.8%)

This means we need to sample about 7.8% of housing units in Cluster 4

Why This Makes Sense Cluster 4 had a higher chance of being selected (64% vs 36.5%), so we need to sample a smaller percentage of its units to balance things out. This creates our EPSEM design where every housing unit in the population ends up with the same 5% chance of being in our final sample.

The final result:

We sample 13.7% of units from Cluster 2 We sample 7.8% of units from Cluster 4
...
Number of units to select = 95 × 0.137 = 13.01 Since we can't select a partial housing unit, we would round to 13 housing units.

## 3. Tables in Question 1 and Question 2 are the sample frame from two strata.

## • For Stratum 1: three clusters were selected, i.e., $𝑛_1$ = 3

## • For Stratum 2: two clusters were selected, i.e., $𝑛_2$ = 2
...
In PPS/PPeS sampling, the "zone size" is the selection interval (k), which is calculated as:

Zone Size (k) = Total MOS ÷ Number of selections

Simple Calculation

Steps For Stratum 1: Total MOS = 550 (sum of all MOS values in Question 1)

Number of selections (n₁) = 3

Zone Size = 550 ÷ 3 = 183.33

For Stratum 2: Total MOS = 400 (sum of all MOS values in Question 2)

Number of selections (n₂) = 2

Zone Size = 400 ÷ 2 = 200

Answer The stratum-specific zone sizes are:
...
Step 1: Identify the Selected Clusters From part 1b, we selected Clusters 1, 3, and 4 in Stratum 1 with:
...
Step 2: Calculate the Selection Probability for Each Cluster Using the formula:

Selection probability = (n × Cluster MOS) ÷ Total MOS
...
Step 3: Calculate the Within-Cluster Sampling Rate Using the formula:

Within-cluster rate = Overall rate ÷ Selection probability
...
Step 4: Calculate the Subsample Size for Each Cluster Since we're asked for the targeted subsample size, we multiply the within-cluster rate by the MOS:
...
we need z is our confidence level value (2.576)

p is our expected proportion (0.5)

margin of error is our precision (0.025)

n = (2.576)² × 0.5(1-0.5) ÷ (0.025)²

n = 6.636 × 0.25 ÷ 0.000625
...
m_opt = √\[(5,000 ÷ 100) × (1-0.594) ÷ 0.594\]

m_opt = √\[50 × 0.406 ÷ 0.594\] m_opt = √\[50 × 0.683\]

m_opt = √34.18 m_opt ≈ 5.85 ≈ 6 housing units per block group
...
Number of block groups = Total units needed ÷ Units per block group

Number of block groups = 106,200 ÷ 6 = 17,700 block groups
...
Problem 1: Missing Addresses (Undercoverage)

The Problem: Commercial address lists often don't include all housing units.

They might miss:

New buildings or recently constructed homes

Certain apartment buildings Housing in lower-income areas

Temporary or non-traditional housing

This means some people have zero chance of being selected for your survey, which can bias your results about health insurance coverage.

Solutions:

Use multiple sources: Combine the commercial list with other lists (like utility records, tax assessor data, or postal service data)

On-site listing: Have field staff create a complete list of all housing units in selected block groups

Adjust your weights: Give more statistical weight to respondents from groups that are likely underrepresented due to missing addresses

Problem 2: Duplicate Addresses

The Problem: Some housing units might appear multiple times in commercial lists because:

The same address is formatted differently (123 Main St vs. 123 Main Street) Both a physical address and PO Box are listed for the same household

Errors in the database creation or maintenance Unit numbers might be inconsistently recorded

Duplicates give some households a higher chance of selection, potentially biasing your results.

Solutions:

De-duplication software: Use specialized software to identify and remove likely duplicates before sampling

Field verification: Have staff check selected addresses to identify duplicates during data collection

Statistical correction: Adjust the sampling weights for any units that were found to be duplicated in the frame
...
Step 1: Find the total number of block groups in the population

Add up all block groups across the five counties:

Total = 250 + 50 + 150 + 25 + 45 = 520 block groups

Step 2: Calculate the proportion of block groups in each county

Wayne: 250 ÷ 520 = 0.481 (48.1%)

Washtenaw: 50 ÷ 520 = 0.096 (9.6%)

Oakland: 150 ÷ 520 = 0.288 (28.8%)

Livingston: 25 ÷ 520 = 0.048 (4.8%)

Genesee: 45 ÷ 520 = 0.087 (8.7%)

Step 3: Calculate the number of block groups to sample from each county Multiply each proportion by the total sample size (30):

Wayne: 0.481 × 30 = 14.43

Washtenaw: 0.096 × 30 = 2.88

Oakland: 0.288 × 30 = 8.64

Livingston: 0.048 × 30 = 1.44

Genesee: 0.087 × 30 = 2.61

Step 4: Round to whole numbers while maintaining total sample size
...
Step 5: Verify the total equals 30 14 + 3 + 9 + 1 + 3 = 30
...
$H$ = Number of strata (5 counties)

$W_h = N_h/N$ = Weight of stratum $h$, based on the proportion of total block groups in stratum $h$

$N_h$ = Total block groups in stratum $h$ $N$ = Total block groups across all strata (520) $n_h$ = Number of sampled block groups in stratum $h$ (Wayne = 14, Washtenaw = 3, Oakland = 9, Livingston = 1, Genesee = 3) $f_h = n_h/N_h$ = Sampling fraction within stratum $h$ $s_h^2$ = Sample variance within stratum $h$, calculated from survey data

For means and proportions, use

stratified sampling variance formulas:

Var(estimate) = Sum across strata \[ Weight²ₕ × (1-fₕ) × Variance within stratum ÷ nₕ \]
