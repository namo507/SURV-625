...
The Problem: Commercial address lists often don't include all housing units. They might miss:
...
The same address is formatted differently (123 Main St vs. 123 Main Street)
Both a physical address and PO Box are listed for the same household
Errors in the database creation or maintenance
Unit numbers might be inconsistently recorded
...
Ages of household members (especially whether 18-29 year olds live there)
...
Insurance status indicators (if available)
Presence of health conditions (if available)
Previous healthcare utilization

3. Housing Characteristics
Home ownership status (owner vs. renter)
Housing type (apartment, house, etc.)
Housing value or rent amount
...
1. For Better Sampling Design
Stratification: Create more efficient samples by dividing addresses into groups with similar characteristics before sampling
Targeted sampling: Focus more on demographics likely to include your key population (18-29 year olds)
Sample size adjustment: Allocate larger samples to areas with more variability in health insurance coverage

2. For Non-Response Adjustments
Create detailed non-response weight adjustments when certain groups are less likely to participate
Form adjustment cells based on combinations of auxiliary variables
Compare respondents to non-respondents to assess potential bias

3. For Post-Survey Weighting
Adjust final sample weights to match known population totals
Implement poststratification or calibration procedures
Reduce biases from unequal response rates across demographic groups

4. For Validation and Analysis
Compare your sample to known population characteristics to verify representativeness
Use as control variables in regression models to improve precision of estimates
Create meaningful subgroup analyses for targeted policy recommendations


Why This Matters
Auxiliary variables help reduce sampling variance, improve representativeness, and address issues like nonresponse or noncoverage bias.
Prioritize variables that are strongly correlated with your survey outcomes (e.g., private health insurance coverage) and are reliably available from the vendor.
By strategically using auxiliary information, you can enhance the accuracy and efficiency of your survey design while staying within budget.
...
Step 3: Calculate the number of block groups to sample from each county
Multiply each proportion by the total sample size (30):
...
Livingston: 1.44 → 1
Genesee: 2.61 → 3
Step 5: Verify the total equals 30
14 + 3 + 9 + 1 + 3 = 30
...
1. Recognize the Design Structure
Our sample has a stratified design with:

5 counties (strata): Wayne, Washtenaw, Oakland, Livingston, and Genesee
30 block groups selected proportionally across these counties
Block groups are the primary sampling units (PSUs)



Step 2: Use Stratified Variance Estimation Formulas
For estimating the population mean or proportion, use variance formulas specific to stratified sampling:
...
$N_h$ = Total block groups in stratum $h$
$N$ = Total block groups across all strata (520)
$n_h$ = Number of sampled block groups in stratum $h$ (Wayne = 14, Washtenaw = 3, Oakland = 9, Livingston = 1, Genesee = 3)
$f_h = n_h/N_h$ = Sampling fraction within stratum $h$
$s_h^2$ = Sample variance within stratum $h$, calculated from survey data

For means and proportions, use stratified sampling variance formulas:
Var(estimate) = Sum across strata [ Weight²ₕ × (1-fₕ) × Variance within stratum ÷ nₕ ]

Where:
Weight₍ₕ₎ = Number of block groups in county h ÷ Total block groups (520)
f₍ₕ₎ = Sampling fraction in county h (number selected ÷ total in county)
n₍ₕ₎ = Number of block groups sampled in county h



Step 3: Calculate Within-Stratum Variances
Compute the sample variance ($s_h^2$) for each variable of interest within each county using data collected from sampled block groups. These variances are crucial for estimating overall sampling error.

Step 4: Apply Finite Population Correction (FPC)
Include the term $(1-f_h)$ to adjust for the finite population size within each stratum. This is particularly important for strata where the sampling fraction is substantial.

Step 5: Determine Degrees of Freedom
For confidence intervals, approximate degrees of freedom as:
...
Livingston: $1-1=0$
Genesee: $3-1=2$
Total degrees of freedom: $df = 13+2+8+0+2=25$

Use approximately 25 degrees of freedom (sum of n₍ₕ₎-1 across counties)
For 95% confidence intervals, use t-distribution critical values rather than normal distribution

The proportional allocation used (Wayne: 14, Washtenaw: 3, Oakland: 9, Livingston: 1, Genesee: 3)
How to properly account for this design in their analyses
Why ignoring the design would lead to incorrect confidence intervals and p-values

Step 6: Use Statistical Software
Recommend analysts use software designed for complex survey designs (e.g., R's survey package, SAS PROC SURVEY procedures, or Stata svy commands). When specifying the design, include:

Strata variables (counties)
PSU variables (block groups)
Any sampling weights (if applicable)
Example R code:
...
Step 7: Weighting Considerations
If analysis focuses on housing units or individuals within block groups, weights may be necessary to account for differences in housing unit counts across sampled block groups. The basic sampling weight would be:
