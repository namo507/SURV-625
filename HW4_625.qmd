...
The intraclass correlation (roh) is actually around 0.14, not 11.5 as incorrectly calculated earlier
With this moderate level of within-cluster homogeneity, increasing the sample size within clusters still provides valuable new information
The SRS variance decreases substantially when the total sample size increases from 94 to 282
The design effect does increase from 2.17 to 4.81 when we triple the cluster sizes, which reflects the diminishing returns of adding more units within the same clusters. However, this increase in design effect is more than offset by the reduction in the base SRS variance.
...
$z_{hi} = t_{y,h,i} - \hat{p} \cdot t_{x,h,i}$ are the residuals
$x = \sum_{h=1}^L \sum_{i=1}^{n_h} t_{x,h,i}$ is the total sample size
...
$V_{cluster}(\hat{p})$ is the variance under cluster sampling from part (a)
$V_{SRS}(\hat{p}) = \frac{p(1-p)}{n-1}$ is the variance under simple random sampling with the finite population correction
